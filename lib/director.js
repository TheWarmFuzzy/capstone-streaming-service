...
function www(views, web_directory){
	...
	.get(function(req,res,next){
		...
		console.log("\t","Status: User - Allowed");
		...
		if(req.query.signature){
			...
			if(signed){
					...
					if(signed.content && signed.content_path){ //The content file
						...
						if(req.authentication.user == signed.user || "undefined" == typeof signed.user){
							...
							if(fs.existsSync(content_path)){
								...
								console.error("Content not found: Video");
								error({
									"status":404,
									"res":res,
									"engine":{
				...
				if("image" == req.params.type){
					//Check what's in the signature
					if(signed.content_path){
						
						//Check if the file exits
						if(fs.existsSync(signed.content_path)){
							
							res.sendFile(signed.content_path);
							
						}else{
							
							console.error("Content not found: Image");
							error({
								"status":404,
								"res":res,
								"engine":{
...
preload.enforce_ssl = function(req, res, next){
	if(!req.secure)
		error({
			...
			"status":406,
			"engine":{
				"error_message":"Your session is insecure, please use a secure connection",
				"user":req.authentication.user
			}
		});
...
preload.authorize = function(req, res, next){
	...
	{
		console.log("\t","Status: Group - Allowed");
