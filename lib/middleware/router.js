...
function get_watch(req, res, next){
	...
	sql.procedure(procedure, function(e, r, f){
		if(e){
			...
			//Video Signature
			var video_signature = signed_urls.create_signature(data);	
			
			//Image Signature
			data.content_path = path.resolve(web_dir + "/uploads/users/" + results.email + "/videos/" + video_id + "/video_preview.jpg")
			var image_signature = signed_urls.create_signature(data);
